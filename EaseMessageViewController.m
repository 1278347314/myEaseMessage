        ...
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            if (imageBody.attachmentDownloadStatus == EMAttachmentDownloadSuccessed)
            {
                ...
                if (localPath && localPath.length > 0) {
                    ...
                    if (image)
                    {
                        [[EaseMessageReadManager defaultManager] showBrowserWithImages:@[image] messageModel:model];
                    }
                    else
                    {
...
- (void)_audioMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if (model.bodyType == eMessageBodyType_Voice) {
        ...
        if (isPrepare) {
            ...
            if (readManager.delegate && [readManager.delegate respondsToSelector:@selector(readMessageFinished:)])
            {
                [readManager.delegate readMessageFinished:model];
            }
            readManager.delegate = nil;
        }
    }
...
- (void)messageCell:(EaseMessageCell *)cell didSelectLinkWithURL:(NSURL *)url {
    if (self.delegate && [self.delegate respondsToSelector:@selector(messageViewController:didSelectLinkWithURL:)]) {
        [self.delegate messageViewController:self.tableView didSelectLinkWithURL:url];
    } else {
